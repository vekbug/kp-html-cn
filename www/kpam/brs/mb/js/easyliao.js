	...
	tag:标签ID
	*/
	function openJesongChatByCus(cId,tar,tag){
		return openJesongChat(cId,'c',tar,tag);
	}
	/*
	打开对话框接入到指定的客服分组
	cId:公司id
	...
	tag:标签ID
	*/
	function openJesongChatByGroup(cId,tar,tag){
		return openJesongChat(cId,'g',tar,tag);
	}
	function openJesongChat(cId,c,tar,tag){
		var param = '';
		if(c == 'c'){
			param += 'n='+tar;
		}else if(c == 'g'){
